 ...
 *  Generated by : subho
 *  Generated on : 2017-01-31 15:33:05.884
 */
...
// Call the method to add the value of testRefMap to the serialized data map.
			...
			/**
			 * Generated code for type comparison utility for test_ref_type.
			 *
			 * Note:: Should not be modified as the changes will be lost when the code is re-generated.
			 */
			class compare_test_ref_type
			{
			public:
				static bool equals ( const test_ref_type * source, const mutable_test_ref_type * target )
				{
					if ( IS_NULL ( source ) ) {
						if ( IS_NULL ( target ) ) {
							return true;
...

					{
						...
						__base_datatype_io *th = nt->get_type_handler ( __record_mode::RM_READ );
						CHECK_NOT_NULL ( th );
						const void *tv = source->get_name();
						const void *sv = target->get_name();
						if ( IS_NULL ( sv ) ) {
							if ( NOT_NULL ( tv ) ) {
								return false;
							}
					...
					{
						// Compare map instance testRefMap
						const unordered_map<std::string, test_type *> *sv = source->get_testRefMap();
						const unordered_map<std::string, mutable_test_type *> *tv = target->get_testRefMap();
						if ( IS_NULL ( sv ) ) {
							if ( NOT_NULL ( tv ) ) {
								return false;
							}
